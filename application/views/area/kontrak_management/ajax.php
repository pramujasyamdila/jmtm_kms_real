<script>
    ...
    function byid(id, type) {
        ...
        $.ajax({
            type: "GET",
            url: "<?= base_url('area/data_kontrak/byid/'); ?>" + id,
            dataType: "JSON",
            success: function(response) {
                if (type == 'detail_kontrak') {
                    window.location.href = "<?= base_url('area/data_kontrak/detail_kontrak/') ?>" + id
        ...
        swal({
            ...
            if (willDelete) {
                if (status == 'non_aktif') {
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('area/data_kontrak/status_control/') ?>" + id_pegawai,
                        dataType: "JSON",
                        data: {
                            status: 0
                        },
    ...
    function save_program() {
        $.ajax({
            method: "POST",
            url: "<?= base_url('area/data_kontrak/add_kontrak/') ?>",
    ...
    function edit_data() {
        ...
        $.ajax({
            method: "POST",
            url: "<?= base_url('area/data_kontrak/update_pegawai/') ?>" + id_pegawai,
    ...
    $(document).ready(function() {
        tabledata.DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('area/data_kontrak/get_data/') ?>",
                "type": "POST"
            },
            "columnDefs": [{
                "target": [-1],
