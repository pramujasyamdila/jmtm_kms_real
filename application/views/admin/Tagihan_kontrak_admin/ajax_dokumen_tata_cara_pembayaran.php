<script>
    ...
    function cari_id_detail_program_penyedia_jasa(id_detail_program_penyedia_jasa) {
        ...
        if (id_detail_program_penyedia_jasa == '') {
            message('No. Kontrak Belum Di isi!', 'primary', 'Gagal Mendapatkan Data!')
        } else {
            $.ajax({
                method: "POST",
                    ...
                    for (i = 0; i < response['get_detail_taggihan'].length; ++i) {
                        ...
                        if (response['vendor_session']) {
                            if (response['get_detail_taggihan'][i].status_penaggihan == 1) {
                                var action = ' <a style="font-size:10px" class="btn btn-sm btn-primary text-white" onclick="Edit_traking(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">Kelola Tata Cara Pembayaran</a>'
                            ...
                            if (response['pegawai']) {
                                if (response['get_detail_taggihan'][i].status_penaggihan == 1) {
                                    ...
                                    if (response['get_detail_taggihan'][i].status_penaggihan == 2 || response['get_detail_taggihan'][i].status_terakhir == 'Pencairan') {
                                        var action = ' <a style="font-size:10px" class="btn btn-block btn-sm btn-primary text-white" onclick="Edit_traking(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">Kelola Tata Cara Pembayaran</a>'
                                    } else {
                                        var action = '<a style="font-size:10px" class="btn btn-sm btn-primary text-white" onclick="Edit_traking(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">Kelola Tata Cara Pembayaran</a>'
                                    }
                                }
                                ...
                                if (response['get_detail_taggihan'][i].status_penaggihan == 1) {
                                    var action = '<a style="font-size:10px" class="btn btn-sm btn-primary text-white" onclick="Traking_area2(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">View Traking <a style="font-size:10px" class="btn btn-sm btn-primary text-white" onclick="Edit_traking(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">Kelola Tata Cara Pembayaran</a></a>'
                                } else {
                                    if (response['get_detail_taggihan'][i].status_penaggihan == 2) {
                                        var action = '<a style="font-size:10px" class="btn btn-sm btn-success text-white" onclick="Traking_area2(' + response['get_detail_taggihan'][i].id_mc + ')" href="javascript:;">Pencairan</a>'
                                    } else {
    ...
    function Edit_traking(id) {
        window.open('<?= base_url('taggihan_kontrak_admin/tagihan_kontrak/tata_cara_pembayaran/') ?>' + id, '_blank');
    }
    ...
    function Simpan_mc() {
        ...
        if ($('[name="tanggal_mc"]').val() == '') {
            message('Harap Isi Tanggal!', 'primary', 'Tanggal Periode Belum Di Isi')
        } else {
            $.ajax({
                method: "POST",
...
<script>
    var form_update_dokumen_ceklist = $('#form_update_dokumen_ceklist')

    function update_dokumen_ceklist() {
        $.ajax({
            method: "POST",
            url: "<?= base_url('taggihan_kontrak_admin/tagihan_kontrak/simpan_update_ceklist_dokumen') ?>",
            data: form_update_dokumen_ceklist.serialize(),
            dataType: "JSON",
            success: function(response) {
                if (response == 'success') {
                    message('success', 'Data Berhasil Di Simpan!', 'Berhasil')
...
<script>
    var modal_dok_mc = $('#modal_dok_mc');
    var form_dok_mc = $('#form_dok_mc');
    var tabledata_dok_mc = $('#tabledata_dok_mc');

    function ModalDokumen(type, title) {
        modal_dok_mc.modal('show');
        var id_detail_program_penyedia_jasa = $('[name="id_detail_program_penyedia_jasa"]').val();
        $('[name="type"]').val(type);
        $('#data_title').html(title);
        tabledata_dok_mc.DataTable({
            "responsive": true,
            "autoWidth": false,
            "processing": true,
            "serverSide": true,
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('taggihan_kontrak_admin/tagihan_kontrak/data_get_dokumen_mc/') ?>" + id_detail_program_penyedia_jasa,
                "type": "POST",
                data: {
                    type: type,
                }
    ...
    form_dok_mc.on('submit', function(e) {
        e.preventDefault();
        if ($('.file_dokumen_penunjang').val() == '') {
            ...
            $.ajax({
                url: "<?= base_url('taggihan_kontrak_admin/tagihan_kontrak/add_dok_mc') ?>",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
                ...
                success: function(response) {
                    ...
                    var timer = setInterval(function() {
                        ...
                        progress_bar_process_dok_mc(percentage, timer);
                    }, 1000);
                }
            });
        }
    ...
    function progress_bar_process_dok_mc(percentage, timer) {
