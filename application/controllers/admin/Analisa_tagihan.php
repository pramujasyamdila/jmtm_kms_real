...
class Analisa_tagihan extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['result_kontrak_select'] = $this->Data_kontrak_model->get_result_kontrak_select($data['id_departemen'], $data['id_area'], $data['id_sub_area']);
		$this->load->view('template_stisla/header');
		$this->load->view('template_stisla/sidebar', $data);
		$this->load->view('admin/analisis_tagihan_global/index', $data);
		$this->load->view('template_stisla/footer',);
		$this->load->view('admin/analisis_tagihan_global/ajax');
	}

	public function filter_kontrak_grafik($id_kontrak)
	{
		$data['title'] = 'Admin/Dashboard';
		// januari
		...
		$this->load->view('template_stisla/header');
		$this->load->view('template_stisla/sidebar', $data);
		$this->load->view('admin/analisis_tagihan_global_filter/index', $data);
		$this->load->view('template_stisla/footer',);
		$this->load->view('admin/analisis_tagihan_global_filter/ajax');
	}
	public function add_pendapatan()
	{
		$id_kontrak =  $this->input->post('id_kontrak_cari_pendapatan');
	...
	public function get_data_pencairan()
	{
		...
		$get_pegawai = $this->Auth_model->get_pegawai();
		$resultss = $this->Taggihan_kontrak_admin_model->getdatatable_pencairan($get_pegawai, $bulan);
		$data = [];
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Taggihan_kontrak_admin_model->count_all_data_pencairan($get_pegawai, $bulan),
			"recordsFiltered" => $this->Taggihan_kontrak_admin_model->count_filtered_data_pencairan($get_pegawai, $bulan),
			"data" => $data
		);
	...
	public function get_data_pendapatan()
	{
		...
		$bulan = $this->input->post('bulan');
		$get_pegawai = $this->Auth_model->get_pegawai();
		$resultss = $this->Taggihan_kontrak_admin_model->getdatatable_pendapatan($get_pegawai, $bulan);
		$data = [];
		$no = $_POST['start'];
		foreach ($resultss as $rs) {
			$row = array();
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Taggihan_kontrak_admin_model->count_all_data_pendapatan($get_pegawai, $bulan),
			"recordsFiltered" => $this->Taggihan_kontrak_admin_model->count_filtered_data_pendapatan($get_pegawai, $bulan),
			"data" => $data
		);
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	}
	...
	{
		$get_pegawai = $this->Auth_model->get_pegawai();
		$result_kontrak = $this->Data_kontrak_model->get_result_kontrak($get_pegawai);
		$passing = [
	...
	function result_by_id_kontrak()
	{
		...
		$get_pegawai = $this->Auth_model->get_pegawai();
		$januari = $this->Taggihan_kontrak_admin_model->get_bulan_januari_grafik($get_pegawai);
		$total_januari = 0;
		...
		// februari
		$februari = $this->Taggihan_kontrak_admin_model->get_bulan_februari_grafik($get_pegawai);
		$total_februari = 0;
		...
		// maret
		$maret = $this->Taggihan_kontrak_admin_model->get_bulan_maret_grafik($get_pegawai);
		$total_maret = 0;
		...
		// april
		$april = $this->Taggihan_kontrak_admin_model->get_bulan_april_grafik($get_pegawai);
		$total_april = 0;
		...
		// mei
		$mei = $this->Taggihan_kontrak_admin_model->get_bulan_mei_grafik($get_pegawai);
		$total_mei = 0;
		...
		// juni
		$juni = $this->Taggihan_kontrak_admin_model->get_bulan_juni_grafik($get_pegawai);
		$total_juni = 0;
		...
		// juli
		$juli = $this->Taggihan_kontrak_admin_model->get_bulan_juli_grafik($get_pegawai);
		$total_juli = 0;
		...
		// agustus
		$agustus = $this->Taggihan_kontrak_admin_model->get_bulan_agustus_grafik($get_pegawai);
		$total_agustus = 0;
		...
		// september
		$september = $this->Taggihan_kontrak_admin_model->get_bulan_september_grafik($get_pegawai);
		$total_september = 0;
		...
		// oktober
		$oktober = $this->Taggihan_kontrak_admin_model->get_bulan_oktober_grafik($get_pegawai);
		$total_oktober = 0;
		...
		// november
		$november = $this->Taggihan_kontrak_admin_model->get_bulan_november_grafik($get_pegawai);
		$total_november = 0;
		...
		// desember
		$desember = $this->Taggihan_kontrak_admin_model->get_bulan_desember_grafik($get_pegawai);
		$total_desember = 0;
		...
		// _pendapatan
		// januari_pendapatan
		$januari_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_januari_pendapatan_grafik($get_pegawai);
		...
		// februari_pendapatan
		$februari_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_februari_pendapatan_grafik($get_pegawai);
		...
		// maret_pendapatan
		$maret_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_maret_pendapatan_grafik($get_pegawai);
		...
		// april_pendapatan
		$april_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_april_pendapatan_grafik($get_pegawai);
		...
		// mei_pendapatan
		$mei_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_mei_pendapatan_grafik($get_pegawai);
		...
		// juni_pendapatan
		$juni_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_juni_pendapatan_grafik($get_pegawai);
		...
		// juli_pendapatan
		$juli_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_juli_pendapatan_grafik($get_pegawai);
		...
		// agustus_pendapatan
		$agustus_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_agustus_pendapatan_grafik($get_pegawai);
		...
		$september_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_september_pendapatan_grafik($get_pegawai);
		...
		// oktober_pendapatan
		$oktober_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_oktober_pendapatan_grafik($get_pegawai);
		...
		// november_pendapatan
		$november_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_november_pendapatan_grafik($get_pegawai);
		...
		// desember_pendapatan
		$desember_pendapatan = $this->Taggihan_kontrak_admin_model->get_bulan_desember_pendapatan_grafik($get_pegawai);
	...
	function result_by_id_kontrak_awal_global()
	{
		$id_kontrak = $this->input->post('id_kontrak');
		$get_pegawai = $this->Auth_model->get_pegawai();
		$januari = $this->Taggihan_kontrak_admin_model->get_bulan_januari_grafik($get_pegawai);
